...
classDiagram 
    class Car {
        -String agentId
        -List carItinerary<ItineraryItem>
        -String startNode
        -String endNode
        -int itineraryIndex
    ...
    class Itinerary {
      -List itineraryItems<ItineraryItem>       
    }
    
    Cars --o Car
    Itinerary --o ItineraryItem
    Car --o Itinerary
```

- All cars (`car`) are added to the `cars` list
- A car has an itinerary (`carItinerary`)
- An itinerary item (`ItineraryItem`) has a `startNode`, an `endNode` and a `waitTime`
