...
NetLogoJar = 'C:/Program Files (x86)/NetLogo 4.0.4/NetLogo.jar'
Scenario = 'tst_rev'
Base = 'towyn'
...
defences = ["rhyl3", "rhyl2", "rhyl1", "n", "m", "l", "k", "j", "i", "h", "g", "f", "e", "d", "c", "b", "a"]

warningtime = ["07:50:00", "07:51:00", "07:52:00" , "07:53:00", "07:54:00"]
...
writenetlogofile('vehicles.txt', vehicles)

for d in defences:
        for wt in warningtime:
                for sc in seed:
                        for sl in sealevel:
                                print "Running for defence " + d.upper() + " failure, storm surge " + str(sl) + "m, evacuation time " + wt + " random seed " + str(sc)
                                writenetlogofile('timeline.txt', timeline(d, wt, sl ) )
                                writexmlfile('behaviour.xml', behaviourxml(sc) )
                                ##writexmlfile('behaviour2.xml', behaviour, s )
                                runheadless('toytown.nlogo', Scenario + '/behaviour.xml')
                                r = Scenario + '/result-breach-' + d + '-' + str(sl) + '-' + ''.join(wt.split(':')) + '-' + str(sc)
